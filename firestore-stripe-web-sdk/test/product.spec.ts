...
describe("Product emulator tests", () => {
  ...
  describe("getProduct()", () => {
    it("should return Product for the specified ID", async () => {
      const product: Product = await getProduct(emulatedPayments, "premium");

      expect(product).to.eql(premium);
      expect(product.prices).to.be.empty;
    });
  ...
  describe("getPrice()", () => {
    it("should return Price for the specified product and price ID", async () => {
      ...
      expect(price).to.include(premiumPrice1);
    });
